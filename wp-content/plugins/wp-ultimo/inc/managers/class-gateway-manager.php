...
class Gateway_Manager extends Base_Manager {
	...
	public function maybe_process_webhooks() {
		...
		if ($gateway && !is_admin() && is_main_site()) {
			...
			try {
				...
				wu_log_add("wu-{$gateway}-webhook-errors", $message, \Psr\Log\LogLevel::ERROR);
			...
			if ($gateway) {
				...
				try {
					...
					*/
					...
					wu_log_add("wu-{$gateway_id}-webhook-errors", $message, \Psr\Log\LogLevel::ERROR);
	...
	public function process_gateway_confirmations() {
		...
		add_filter('body_class', function($classes) {
			return array_merge($classes, array('wu-process-confirmation'));
		});
		...
		if (!empty($output)) {
			 ...
			 */
			add_action('wu_bypass_checkout_form', function($bypass, $atts) use ($output) {
