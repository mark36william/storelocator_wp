...
class Signup_Field_Template_Selection extends Base_Signup_Field {
	...
	public function get_fields() {
		...
		$editor_fields['template_selection_all'] = array(
			'type'      => 'toggle',
			'title'     => __('Use All Templates', 'wp-ultimo'),
			'desc'      => __('Toggle if you want all templates to be available.', 'wp-ultimo'),
			'value'     => 0,
			'order'     => 21,
			'html_attr' => array(
				'v-model' => 'template_selection_all',
			),
		);
	...
	public function to_fields_array($attributes) {
		...
		if (wu_get_isset($attributes, 'template_selection_template') === 'legacy') {
			...
			wp_register_script('wu-legacy-signup', wu_get_asset('legacy-signup.js', 'js'), array('jquery', 'wu-functions'), wu_get_version());
		...
		$site_list = wu_get_isset($attributes, 'template_selection_all') ? wu_get_site_templates(array('fields' => 'blog_id')) : explode(',', $attributes['template_selection_sites']);
