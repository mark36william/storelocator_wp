	...
	public function on_add_subdomain($subdomain, $site_id) {
		...
		foreach ($domains_to_send as $subdomain) {
			...
			if (is_wp_error($results)) {
				...
				wu_log_add('integration-cloudflare', sprintf('Failed to add subdomain "%s" to Cloudflare. Reason: %s', $subdomain, $results->get_error_message()), \Psr\Log\LogLevel::ERROR);
	...
	public function on_remove_subdomain($subdomain, $site_id) {
		...
		foreach ($domains_to_remove as $original_subdomain) {
			...
			if (is_wp_error($results)) {
				...
				wu_log_add('integration-cloudflare', sprintf('Failed to remove subdomain "%s" to Cloudflare. Reason: %s', $subdomain, $results->get_error_message()), \Psr\Log\LogLevel::ERROR);
