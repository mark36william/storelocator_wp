	...
	public function on_add_domain($domain, $site_id) {
		...
		foreach ($domains as $_domain) {
			...
			if (is_wp_error($response)) {
				...
				wu_log_add('integration-wpmudev', sprintf(__('An error occurred while trying to add the custom domain %s to WPMU Dev hosting.', 'wp-ultimo'), $_domain), \Psr\Log\LogLevel::ERROR);
			...
			if ($body->message) {
				...
				wu_log_add('integration-wpmudev', sprintf(__('An error occurred while trying to add the custom domain %1$s to WPMU Dev hosting: %2$s', 'wp-ultimo'), $_domain, $body->message->message), \Psr\Log\LogLevel::ERROR);
