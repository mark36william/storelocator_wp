...
class Migrator extends Base_Installer {
	...
	public function on_shutdown($session, $dry_run, $installer) {
		...
		wu_log_add(Migrator::LOG_FILE_NAME, sprintf($message, $installer), \Psr\Log\LogLevel::ERROR);
	...
	public function handle($status, $installer, $wizard) {
		...
		*/
		...
		if (is_wp_error($this->server_bypass_status)) {
			...
			wu_log_add(Migrator::LOG_FILE_NAME, $log_message, \Psr\Log\LogLevel::ERROR);
		...
		try {
			 ...
			 */
			wu_log_add(Migrator::LOG_FILE_NAME, $e->__toString(), \Psr\Log\LogLevel::ERROR);
	...
	public function log_ids_of_interest() {
		...
		if (!is_array($this->ids_of_interest)) {
			...
			wu_log_add(Migrator::LOG_FILE_NAME, 'The list of IDs of interested got corrupted, this might indicate problems with one or more migrations', \Psr\Log\LogLevel::ERROR);
		...
		foreach ($this->ids_of_interest as $key => $ids) {
			...
			wu_log_add(Migrator::LOG_FILE_NAME, $message, \Psr\Log\LogLevel::WARNING);
