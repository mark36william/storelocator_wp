...
class Checkout_Form extends Base_Model {
	...
	public function get_steps_to_show() {
		...
		foreach ($steps as $key => $step) {
			...
			$data_fields = array_filter($step['fields'], function ($field) use ($non_data_fields) {

				return !in_array($field['type'], $non_data_fields, true);

			});

			$not_hidden_fields = array_filter($data_fields, function ($field) use ($hidden_fields) {

				return !in_array($field['type'], $hidden_fields, true) && !wu_should_hide_form_field($field);
