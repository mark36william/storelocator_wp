...
class Membership extends Base_Model {
	...
	public function get_discount_code() {
		...
		if (empty($this->discount_code) && !$this->get_meta('verified_payment_discount')) {
			...
			$original_payment = wu_get_payments(array(
				'number'        => 1,
				'membership_id' => $this->get_id(),
				'orderby'       => 'id',
				'order'         => 'ASC',
	...
	protected function has_product_changes() {
		...
		$compiled_old = array_map(function ($product) {

			return $product['product']->get_id() . '|' . $product['quantity'];

		}, $old_products);

		$compiled_new = array_map(function ($product) {
