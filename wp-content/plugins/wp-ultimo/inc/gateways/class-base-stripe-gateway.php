...
class Base_Stripe_Gateway extends Base_Gateway {
	...
	public function process_membership_update(&$membership, $customer) {
		...
		try {
			...
			$existing_items = array_map(function($item) {
	...
	protected function create_recurring_payment($membership, $cart, $payment_method, $s_customer) {
		...
		if ($cart->has_trial()) {
			...
			$fees = array_filter($cart->get_line_items_by_type('fee'), function($fee) {

				return !$fee->is_recurring();
			...
			if (!empty($s_fees)) {
				...
				$sub_options = array(
					'idempotency_key' => wu_stripe_generate_idempotency_key(array_merge(
						array('s_subscription' => $subscription->id),
						$options
					)),
				);
	...
	protected function get_stripe_coupon($coupon_data) {
		...
		try {
			...
			wu_log_add('stripe', sprintf('Error creating Stripe coupon. Code: %s; Message: %s', $error_code, $e->getMessage()), \Psr\Log\LogLevel::ERROR);
	...
	protected function get_ultimo_line_items_from_invoice($invoice_line_items) {
		...
		foreach ($invoice_line_items as $s_line_item) {
			...
			$currency = strtoupper($s_line_item->currency);
			...
			$title = preg_replace($description_pattern, '$1', $s_line_item->description);
	...
	protected function save_payment_method($payment_intent, $s_customer) {
		...
		try {
			...
			wu_log_add('stripe', $error, \Psr\Log\LogLevel::ERROR);
	...
	public function maybe_cancel_old_subscriptions($s_customer) {
		...
		try {
			...
			wu_log_add('stripe', sprintf( 'Stripe Gateway: Subscription cleanup failed for customer #%d. Message: %s', $this->customer->get_id(), $e->getMessage()), \Psr\Log\LogLevel::ERROR);
	...
	public function process_cancellation($membership, $customer) {
		...
		if (!empty($subscription_id)) {
			...
			try {
				...
				wu_log_add('stripe', sprintf('Stripe Gateway: Failed to cancel subscription %s. Message: %s', $subscription_id, $e->getMessage()), \Psr\Log\LogLevel::ERROR);
	...
	public function register_scripts() {
		...
		$obj_name = 'wu_' . str_replace('-', '_', $this->get_id());
	...
	public function maybe_create_plan($args) {
		...
		try {
			...
			wu_log_add('stripe', sprintf('Error creating Stripe plan. Code: %s; Message: %s', $error_code, $e->getMessage()), \Psr\Log\LogLevel::ERROR);
	...
	private function maybe_create_product($name, $id = '') {
		...
		try {
			...
			wu_log_add('stripe', sprintf('Error creating Stripe product. Code: %s; Message: %s', $error_code, $e->getMessage()), \Psr\Log\LogLevel::ERROR);
