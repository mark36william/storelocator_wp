...
class PayPal_Gateway extends Base_Gateway {
	...
	public function process_membership_update(&$membership, $customer) {
		...
		$description = wu_get_setting('company_name', __('Subscription', 'wp-ultimo')) . ': ' . implode(', ', array_map(function($item) {

			return 'x' . $item->get_quantity() . ' ' . $item->get_title();

		}, $temp_payment->get_line_items()));
