...
class Screenshot {
	...
	public static function save_image_from_url($url) {
		...
		if (wp_remote_retrieve_response_code($response) !== 200) {
			...
			wu_log_add('screenshot-generator', $log_prefix . wp_remote_retrieve_response_message($response), \Psr\Log\LogLevel::ERROR);
		...
		if (is_wp_error($response)) {
			...
			wu_log_add('screenshot-generator', $log_prefix . $response->get_error_message(), \Psr\Log\LogLevel::ERROR);
		...
		if (strpos($response['body'], "\x89\x50\x4e\x47\x0d\x0a\x1a\x0a") !== 0) {
			...
			wu_log_add('screenshot-generator', $log_prefix . __('Result is not a PNG file.', 'wp-ultimo'), \Psr\Log\LogLevel::ERROR);
		...
		if (!empty($upload['error'])) {
			...
			wu_log_add('screenshot-generator', $log_prefix . json_encode($upload['error']), \Psr\Log\LogLevel::ERROR);
