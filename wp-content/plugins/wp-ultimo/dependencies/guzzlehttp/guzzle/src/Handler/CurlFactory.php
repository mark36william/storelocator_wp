...
namespace WP_Ultimo\Dependencies\GuzzleHttp\Handler;

use WP_Ultimo\Dependencies\GuzzleHttp\Exception\ConnectException;
use WP_Ultimo\Dependencies\GuzzleHttp\Exception\RequestException;
use WP_Ultimo\Dependencies\GuzzleHttp\Promise as P;
use WP_Ultimo\Dependencies\GuzzleHttp\Promise\FulfilledPromise;
use WP_Ultimo\Dependencies\GuzzleHttp\Promise\PromiseInterface;
use WP_Ultimo\Dependencies\GuzzleHttp\Psr7\LazyOpenStream;
use WP_Ultimo\Dependencies\GuzzleHttp\TransferStats;
use WP_Ultimo\Dependencies\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \WP_Ultimo\Dependencies\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \WP_Ultimo\Dependencies\GuzzleHttp\Psr7\Utils::streamFor($sink);
